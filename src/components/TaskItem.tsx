...
const TaskItem: React.FC<TaskItemProps> = ({ task, onUpdate, onDelete }) => {
  const [isExpanded, setIsExpanded] = useState(true); // Each task manages its own expanded state
  ...
  const styles: { [key: string]: React.CSSProperties } = {
    ...
    detailsContainer: { maxHeight: isExpanded ? '1000px' : '0', overflow: 'hidden', transition: 'max-height 0.5s ease-in-out' },
    taskDetails: { margin: '10px 0 15px 0', color: '#4B5563', whiteSpace: 'pre-wrap', lineHeight: '1.6' },
  ...
  return (
    <div style={{ ...styles.taskCard, opacity: task.status === 'COMPLETED' ? 0.6 : 1 }}>
      <div style={styles.taskHeader}>
        ...
        <div style={styles.actions}>
          ...
          <button onClick={() => onDelete(task.id)} style={{...styles.button, color: '#DC2626'}}>
              <TrashIcon /> Delete
          </button>
          <button onClick={() => setIsExpanded(!isExpanded)} style={styles.foldButton} aria-label={isExpanded ? "Collapse task" : "Expand task"}>
