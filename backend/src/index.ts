...
const syncCalendarForCourse = async (db: FirebaseFirestore.Firestore, userId: string, courseId: string, courseData: any) => {
    ...
    if (courseData.schedule && courseData.termStartDate && courseData.termEndDate) {
        // UPDATED: Use the timezone offset stored in the course data.
        const offset = (typeof courseData.timezoneOffset === 'number') ? courseData.timezoneOffset : 0;

        const weekDays: { [key: string]: number } = { 'Sunday': 0, 'Monday': 1, 'Tuesday': 2, 'Wednesday': 3, 'Thursday': 4, 'Friday': 5, 'Saturday': 6 };
        
        const termStart = new Date(courseData.termStartDate + 'T00:00:00Z');
        const termEnd = new Date(courseData.termEndDate + '-01T00:00:00Z');
        termEnd.setUTCMonth(termEnd.getUTCMonth() + 1, 0);

        courseData.schedule.forEach((classItem: any) => {
            const targetDay = weekDays[classItem.day];
            if (targetDay === undefined) return;

            let currentDate = new Date(termStart.getTime());
            currentDate.setUTCDate(currentDate.getUTCDate() + (targetDay - currentDate.getUTCDay() + 7) % 7);
            ...
            while (currentDate <= termEnd) {
                ...
                const eventStartUTC = Date.UTC(currentDate.getUTCFullYear(), currentDate.getUTCMonth(), currentDate.getUTCDate(), startHour, startMinute);
                const eventEndUTC = Date.UTC(currentDate.getUTCFullYear(), currentDate.getUTCMonth(), currentDate.getUTCDate(), endHour, endMinute);
                
                const eventStartTimestamp = eventStartUTC - (offset * 60000);
                const eventEndTimestamp = eventEndUTC - (offset * 60000);

                const newEvent = {
                    title: courseData.name,
                    startTime: new Date(eventStartTimestamp),
                    endTime: new Date(eventEndTimestamp),
                    courseId: courseId,
                    autoGenerated: true
                };
...
app.post('/courses', authenticate, async (req, res) => {
    try {
        const { user } = req as any;
        const { name, code, generationType, schedule, termStartDate, termEndDate, timezoneOffset } = req.body;
        ...
        const db = getFirestore();
        // UPDATED: Save timezoneOffset with the new course document
        const newCourse = { 
            name, 
            code: code || '', 
