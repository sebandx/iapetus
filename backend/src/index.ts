...
const syncCalendarForCourse = async (db: FirebaseFirestore.Firestore, userId: string, courseId: string, courseData: any) => {
    ...
    if (courseData.schedule && courseData.termStartDate && courseData.termEndDate) {
        ...
        // Convert client's offset (e.g., 240 for EDT) to a standard timezone string (e.g., "-04:00")
        const sign = offset > 0 ? '-' : '+';
        ...
        courseData.schedule.forEach((classItem: any) => {
            ...
            while (currentDate <= termEnd) {
                ...
                // Construct full ISO 8601 strings with the correct timezone offset
                const startISO = `${dateString}T${classItem.startTime}:00${timezoneString}`;
                const endISO = `${dateString}T${classItem.endTime}:00${timezoneString}`;
                
                // Let the Date constructor parse the unambiguous string to the correct UTC timestamp
                const eventStart = new Date(startISO);
                const eventEnd = new Date(endISO);

                // Add a validity check to prevent errors
                if (isNaN(eventStart.getTime()) || isNaN(eventEnd.getTime())) {
                    console.error("Skipping invalid date created from ISO string:", { startISO, endISO });
...
app.post('/courses', authenticate, async (req, res) => {
    try {
        ...
        const db = getFirestore();
        // UPDATED: Save the timezone offset with the course document
        const newCourse = { 
            name, 
            code: code || '', 
            ...
            timezoneOffset: (typeof timezoneOffset === 'number') ? timezoneOffset : new Date().getTimezoneOffset()
        };
